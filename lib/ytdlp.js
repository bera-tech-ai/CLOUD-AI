 ...
 *           SoundCloud, Reddit, Vimeo, CapCut, and 1000+ more sites.
 */
...
export const YTDLP = process.env.YTDLP_PATH || '/home/runner/bin/yt-dlp';
 ...
 * Returns: { file, title, duration, durationSec, uploader, views, thumbnail, url }
 */
export async function downloadAudio(url, options = {}) {
  // Get metadata first (fast, separate call)
  ...
  const args = [
    ...
    '-x',
    '--audio-format', 'mp3',
    '--audio-quality', options.quality || '5',   // 5 = ~128kbps good quality
    '-o', out,
    '--no-warnings',
    '--no-progress',
  ];
...
export async function downloadVideo(url, options = {}) {
  ...
  try {
    ...
    // Search for the actual output file
    const files = fs.readdirSync(TMP).filter(f => f.startsWith(id));
    if (files.length === 0) throw new Error('Video file not found after download');
